<?php // HTML and page generation functions.

class Pages {
	private $data_accessor;

	function __construct($data_accessor) {
	...
	function openBoard($uri) {
		...
		$b = $this->data_accessor->get_board_info($uri);
		if ($b) {
			setupBoard($b);
	...
	function boardTitle($uri) {
		return $this->data_accessor->get_board_title($uri);
	}
	...
	function listBoards($just_uri = false) {
		if ($just_uri) {
			return $this->data_accessor->get_boards_uris();
		} else {
			return $this->data_accessor->get_boards();
		}
	}
	...
	function displayBan($ban) {
		...
		if ($ban['post'] && isset($ban['post']['board'], $ban['post']['id'])) {
			if (openBoard($ban['post']['board'])) {
				$files = $this->data_accessor->get_ban_files_of($ban['post']['id'], $ban['post']['board']);
				foreach ($files as $f) {
					$ban['post'] = array_merge($ban['post'], $f);
				}
			}
			if ($ban['post']['thread']) {
		...
		if ($config['ban_appeals']) {
			$appeals = $this->data_accessor->get_ban_appeals((int)$ban['id']);
			foreach ($appeals as $ban_appeal) {
	...
	function threadLocked($id) {
		...
		return (bool)$this->data_accessor->get_thread_locked($id, $board);
	}
	...
	function threadSageLocked($id) {
		...
		return (bool)$this->data_accessor->get_thread_sage($id, $board);
	}
	...
	/**
	 * @param bool $brief Don't need to generate anything yet.
	 */
	...
	/**
	 * Check the "Domain Name System Blacklists"
	 */
	function checkDNSBL() {
		global $config;

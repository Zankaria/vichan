...
function link_for($post, $page50 = false, $foreignlink = false, $thread = false) {
	...
	if ( $page50 &&  $slug) {
		$tpl = $config['file_page50_slug'];
	} elseif (!$page50 &&  $slug) {
		$tpl = $config['file_page_slug'];
	} elseif ( $page50 && !$slug) {
		$tpl = $config['file_page50'];
	} elseif (!$page50 && !$slug) {
		$tpl = $config['file_page'];
	}
...
function check_thread_limit($post) {
	global $config, $board;
	if (!isset($config['max_threads_per_hour']) || !$config['max_threads_per_hour']) {
		return false;
	}
